...
- (void)setShareView
{
    if (m_shareView == nil) {
        ...
        m_shareView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 280, 200)];
        ...
        if ([GameCommon getNewStringWithId:KISDictionaryHaveKey(self.dataDic, @"title")].length > 0) {
            titleSize = [[GameCommon getNewStringWithId:KISDictionaryHaveKey(self.dataDic, @"title")] sizeWithFont:[UIFont boldSystemFontOfSize:15.0] constrainedToSize:CGSizeMake(260, 50)];
        }
...
//        float titleHeg = titleSize.height > 50 ? 50 : titleSize.height;
        ...
        if ([GameCommon getNewStringWithId:KISDictionaryHaveKey(self.dataDic, @"thumb")].length > 0 && ![[GameCommon getNewStringWithId:KISDictionaryHaveKey(self.dataDic, @"thumb")] isEqualToString:@"null"]) {
            EGOImageView* thumb = [[EGOImageView alloc] initWithFrame:CGRectMake(10, titleSize.height + 10, 50, 50)];
...
//            CGSize contentSize = [KISDictionaryHaveKey(self.dataDic, @"msg") sizeWithFont:[UIFont boldSystemFontOfSize:13.0] constrainedToSize:CGSizeMake(200, 80)];
            UILabel* contentLabel = [CommonControlOrView setLabelWithFrame:CGRectMake(70, titleSize.height + 10, 170, 80) textColor:kColorWithRGB(102, 102, 102, 1.0) font:[UIFont boldSystemFontOfSize:13.0] text:KISDictionaryHaveKey(self.dataDic, @"msg") textAlignment:NSTextAlignmentLeft];
            contentLabel.numberOfLines = 0;
            [m_shareView addSubview:contentLabel];
        }
        else
        {
//            CGSize contentSize = [KISDictionaryHaveKey(self.dataDic, @"msg") sizeWithFont:[UIFont boldSystemFontOfSize:13.0] constrainedToSize:CGSizeMake(260, 80)];

            UILabel* contentLabel = [CommonControlOrView setLabelWithFrame:CGRectMake(10, titleSize.height + 10, 260, 80) textColor:kColorWithRGB(102, 102, 102, 1.0) font:[UIFont boldSystemFontOfSize:13.0] text:KISDictionaryHaveKey(self.dataDic, @"msg") textAlignment:NSTextAlignmentLeft];
            contentLabel.numberOfLines = 0;
            [m_shareView addSubview:contentLabel];
        }
        ...
        UIButton* sendBtn = [[UIButton alloc] initWithFrame:CGRectMake(145, 155, 120, 35)];
